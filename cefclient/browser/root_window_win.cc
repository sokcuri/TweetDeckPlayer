...
#include "cefclient/resources/win/resource.h"
...
void RootWindowWin::Init(RootWindow::Delegate* delegate,
  ...
  start_rect_.left = GetPrivateProfileInt(L"window", L"left", bounds.x, path.c_str());
  start_rect_.top = GetPrivateProfileInt(L"window", L"top", bounds.y, path.c_str());
  start_rect_.right = GetPrivateProfileInt(L"window", L"right", bounds.x + bounds.width, path.c_str());
  start_rect_.bottom = GetPrivateProfileInt(L"window", L"bottom", bounds.y + bounds.height, path.c_str());

  int val;
  val = GetPrivateProfileInt(L"setting", L"DefaultAlwaysOnTop", 0, path.c_str());
  WritePrivateProfileString(L"setting", L"DefaultAlwaysOnTop", std::to_wstring((val == 1 ? 1 : 0)).c_str(), path.c_str());
  
  always_on_top_ = (val == 1 ? true : false);
  /*
...
void RootWindowWin::CreateRootWindow(const CefBrowserSettings& settings) {
  ...
  // insert submenu
  HMENU systemMenu = GetSystemMenu(hwnd_, false);
  if (systemMenu)
  {
	  InsertMenu(systemMenu, (UINT)-1, MF_SEPARATOR, 0, 0);
	  InsertMenu(systemMenu, (UINT)-1, MF_BYCOMMAND, ID_ALWAYSONTOP, L"Always On Top");
  }

  // is default top of window
  if (always_on_top_)
  {
...
void SaveINI(HWND hWnd)
{
	...
	WritePrivateProfileString(L"window", L"left", std::to_wstring(rect.left).c_str(), path.c_str());
	WritePrivateProfileString(L"window", L"right", std::to_wstring(rect.right).c_str(), path.c_str());
	WritePrivateProfileString(L"window", L"top", std::to_wstring(rect.top).c_str(), path.c_str());
	WritePrivateProfileString(L"window", L"bottom", std::to_wstring(rect.bottom).c_str(), path.c_str());
}
  ...
  switch (message) {
	...
	case WM_SYSCOMMAND:
	{
		switch (wParam)
		{
			case ID_ALWAYSONTOP:
			{
				HMENU hMenu = GetSystemMenu(self->hwnd_, false);
				MENUITEMINFO info;
				info.cbSize = sizeof(MENUITEMINFO);
				info.fMask = MIIM_STATE;
				if (self->always_on_top_) // en -> dis
				{
					self->always_on_top_ = false;
					info.fState = MFS_UNCHECKED;
					SetMenuItemInfo(hMenu, ID_ALWAYSONTOP, false, &info);

					SetWindowPos(self->hwnd_, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE);
				}
				else // dis -> en
				{
					self->always_on_top_ = true;
					info.fState = MFS_CHECKED;
					SetMenuItemInfo(hMenu, ID_ALWAYSONTOP, false, &info);

					SetWindowPos(self->hwnd_, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE);
				}
				return 0;
			}
